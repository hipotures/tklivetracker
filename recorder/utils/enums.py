...
class TikTokError(Enum):
    ...
    COUNTRY_BLACKLISTED = (
        'Captcha required or country blocked. Use a VPN, room_id, or '
        'authenticate with cookies configured in config.yaml or --cookie-file.'
    )

    COUNTRY_BLACKLISTED_AUTO_MODE = (
        'Automatic mode is available only in unblocked countries. '
        'Use a VPN or authenticate with configured cookies.'
    )

    ACCOUNT_PRIVATE = (
        'Account is private; login is required. Configure an authenticated '
        'cookie file in config.yaml or with --cookie-file.'
    )

    ACCOUNT_PRIVATE_FOLLOW = 'This account is private. Follow the creator to access their LIVE.'

    LIVE_RESTRICTION = (
        'Live is private; login is required. Configure an authenticated '
        'cookie file in config.yaml or with --cookie-file.'
    )
